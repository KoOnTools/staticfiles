    ...
    // Alamat URL API
    let _API_URL_ = 'https://agckey.my.id/api/agoda';
    ...
    if (id = /\/(\?|)\b(\d+)\b(.*)/.exec(pathname)) {
        ...
        if (urlParams.has('search')) {
            ...
            end_point = `${_API_URL_}?offset=${_OFFSET_}&q=${encodeURIComponent(search_keyword)}`;
            ...
            // end point home
            end_point = `${_API_URL_}?offset=${_OFFSET_}`;
            // check end point search page + page number
            if (urlParams.has('page')) {
                // end point search page + page number
                page_number = urlParams.get('page');
                        ...
                        <nav aria-label="Page navigation example">
                            ...
                            agoda.data.links.forEach(paginate => {
                                ...
                                if (paginate.url != null && paginate.active == true) {
                                    ...
                                    pageActive = '';
                                    if ( q_search == "" ) {
                                        pageUrl = ' href="' + paginate.url.replace(_API_URL_, window.location.origin).replace(/\b\?page=1\b/, "") + '"';
                                    } else {
                                        pageUrl = ' href="' + paginate.url.replace(_API_URL_ + "?", window.location.origin + q_search).replace(/page=1\b/, "").replace("page=", "&page=") + '"';
                                    }
                                }
